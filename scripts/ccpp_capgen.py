...
###############################################################################
    ...
    host_files = create_file_list(run_env.host_files, ['meta'], 'Host',
                                  run_env.logger)
    timing_info.append(time.time())
    timing_label.append('Create Host file lists')
    ...
    # end if
    timing_info.append(time.time())
    timing_label.append('Add constituent file to Host file lists')
    scheme_files = create_file_list(run_env.scheme_files, ['meta'],
                                    'Scheme', run_env.logger)
    timing_info.append(time.time())
    timing_label.append('Create Scheme file lists')
    ...
    timing_info.append(time.time())
    timing_label.append('Create SDF file lists')
    ...
    # end if
    timing_info.append(time.time())
    timing_label.append('Write suite caps')
    ...
    # end if
    timing_info.append(time.time())
    timing_label.append('Create kinds file')
    ...
    if run_env.ccpp_cfgfile:
        ...
        # DJS2026: Capgen uses the datatable to create the "cap files". This grabs all needed dependencies,
        #          which includes the "kinds" file and "framework source" files? WHY?
        #          Should <frmwrk_files>, hardcoded below, actually belong in src/CMakeLists.txt?
        kinds_file = list(kinds_file.split())
        framework_files = []
        frmwrk_files = ["ccpp_hashable.F90","ccpp_hash_table.F90","ccpp_constituent_prop_mod.F90","ccpp_scheme_utils.F90"]
        for frmwkr_file in frmwrk_files:
            framework_files.append(_SRC_ROOT+'/'+frmwkr_file)
        # end for
    ...
    timing_info.append(time.time())
    timing_label.append('Build system configuration')
    time_info = [0]
    for index, x in enumerate(timing_info):
        if index > 0:
            time_info.append(timing_info[index] - timing_info[index-1])
        # end if
    # end for
    return time_info, timing_label
...
###############################################################################
    ...
    if framework_env.clean:
        ...
        timing_info, timing_label = capgen(framework_env)
    # end if (clean)
    
    print('-'*50)
    for index, timing in enumerate(timing_info):
        if (index > 0):
            print("{} : {:.2f} seconds".format(timing_label[index].ljust(40), timing_info[index]))
        # end if
    # end for
